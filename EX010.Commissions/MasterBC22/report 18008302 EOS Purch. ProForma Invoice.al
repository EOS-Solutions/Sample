report 18008302 "EOS Purch. ProForma Invoice"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem(PageCounter; "Integer")
            {
                ...
                column(STRSUBSTNO___1__FORMAT_CurrReport_PAGENO__; 1)
                {
                }
                ...
                dataitem(CopyLoop; "Integer")
                {
                    ...
                    {
                        DataItemTableView = SORTING(Number);
                        column(STRSUBSTNO___1__FORMAT_CurrReport_PAGENO___Control18004100; 1)
                        {
                        }
                        ...
                        /*column(PurchWithhContr__ENASARCO_SalesPerson_Amount_; -PurchWithhContr."ENASARCO SalesPerson Amount")
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }*/
                        /*column(x___FORMAT__ROUND_PurchWithhContr__ENASARCO_SalesPerson____0_01________; ' x ' + Format(Round(PurchWithhContr."ENASARCO SalesPerson %", 0.01)) + '% =')
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }*/
                        /*column(x_____FORMAT_ROUND_PurchWithhContr__ENASARCO____0_01______; ' x ' + Format(Round(PurchWithhContr."ENASARCO %", 0.01)) + '%')
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        } */

                        column(NetTotal; (VATAmount + VATBaseAmount) /*- PurchWithhContr."Withholding Tax Amount" - PurchWithhContr."ENASARCO SalesPerson Amount"*/)
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        }

                        /* column(Impon_Contributivo___FORMAT_ROUND_PurchWithhContr__ENASARCO_Contribution_Base__0_01__; 'Impon.Contributivo:' + Format(Round(PurchWithhContr."ENASARCO Contribution Base", 0.01)))
                        {
                            AutoFormatExpression = "Purchase Header"."Currency Code";
                            AutoFormatType = 1;
                        } */
                        ...
                        begin
                            ...
                            if not "Purchase Header"."Prices Including VAT" and ("Purchase Line"."VAT Calculation Type" = "Purchase Line"."VAT Calculation Type"::"Full VAT") then
                            ...
                            then begin
                                if "Purchase Line"."Document Type" = "Purchase Line"."Document Type"::"Credit Memo" then begin
                                    if ("Purchase Line"."Return Qty. to Ship" <> "Purchase Line".Quantity) and ("Purchase Line"."Return Shipment No." = '') then
                                        AddError(StrSubstNo(Text019, "Purchase Line".FieldCaption("Purchase Line"."Return Qty. to Ship"), "Purchase Line".Quantity));
                                    if "Purchase Line"."Qty. to Invoice" <> "Purchase Line".Quantity then
                                        AddError(StrSubstNo(Text019, "Purchase Line".FieldCaption("Purchase Line"."Qty. to Invoice"), "Purchase Line".Quantity));
                                end;
                                ...
                                if "Purchase Line"."Document Type" = "Purchase Line"."Document Type"::Invoice then begin
                                    if ("Purchase Line"."Qty. to Receive" <> "Purchase Line".Quantity) and ("Purchase Line"."Receipt No." = '') then
                                        AddError(StrSubstNo(Text019, "Purchase Line".FieldCaption("Purchase Line"."Qty. to Receive"), "Purchase Line".Quantity));
                                    if "Purchase Line"."Qty. to Invoice" <> "Purchase Line".Quantity then
                                        AddError(StrSubstNo(Text019, "Purchase Line".FieldCaption("Purchase Line"."Qty. to Invoice"), "Purchase Line".Quantity));
                                end;
                            ...
                            if "Purchase Line".Quantity <> 0 then begin
                                ...
                                if "Purchase Line".Amount <> 0 then
                                    AddError(StrSubstNo(Text021, "Purchase Line".FieldCaption("Purchase Line".Amount), "Purchase Line".FieldCaption("Purchase Line".Quantity)));
                            ...
                            if ("Purchase Line".Type.AsInteger() >= "Purchase Line".Type::"G/L Account".AsInteger()) and ("Purchase Line"."Qty. to Invoice" <> 0) then
                                ...
                                if not GroupLines then begin
                                    ...
                                    if (GroupLines and ShowDetails) then begin
                                        ...
                                        if TempPurchLine2.FindFirst then begin
                                            ...
                                            if TempPurchLine2.Type.AsInteger() = 0 then begin
            ...
            if PurchRcptLine.Find('-') then
                repeat
                    ...
                    if PurchLine."Qty. to Invoice" * PurchRcptLine.Quantity < 0 then
                        AddError(StrSubstNo(Text023, PurchLine2.FieldCaption(PurchLine2."Qty. to Invoice")));
            ...
            if ReturnShptLine.Find('-') then
                repeat
                    ...
                    if (-PurchLine."Qty. to Invoice") * ReturnShptLine.Quantity < 0 then
                        AddError(StrSubstNo(Text025, PurchLine2.FieldCaption(PurchLine2."Qty. to Invoice")));
