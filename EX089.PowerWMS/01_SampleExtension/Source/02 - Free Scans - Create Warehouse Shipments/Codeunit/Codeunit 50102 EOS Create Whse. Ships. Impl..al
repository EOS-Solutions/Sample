codeunit 50102 "EOS Create Whse. Ships. Impl." implements "EOS089 WMS Activity Interface V5"
{
    ...
    begin
        exit(Enum::"EOS089 WMS Activity Type"::EOSCreateWhseShipments);  // Change with the corresponding "Activity Type" value
    end;
    ...
    /*
    local procedure GetPostedSourceMessage(DocCreated: Integer; PostedSourceNo: Text)
    var
        EOS089WMSReturnValuesMgmt: Codeunit "EOS089 WMS Return Values Mgmt.";
        SourceCreatedLbl: Label 'Created %1 Warehouse Shipments', Comment = '%1: Warehouse Shipments Counter';    // Change message text
    begin
        Clear(EOS089WMSReturnValuesMgmt);
        EOS089WMSReturnValuesMgmt.PrepareReturnValues();
        EOS089WMSReturnValuesMgmt.SetMessage(StrSubstNo(SourceCreatedLbl, DocCreated));
    ...
    */
    ...
    var
        ...
        LocationFilter: Text;
        CurrentView: Text;
        ScannerSetupCodeLbl: label 'DOCUMENTNO', Locked = true;
    begin
...
#pragma warning restore AA0150
    ...
    begin
        ...
        TempEOS089WMSSourceInformation."Force Avalable Quantity" := true;
        TempEOS089WMSSourceInformation."Force Zero Quantity" := true;
    end;
    ...
    var
        ...
        EOS089WMSReturnValuesMgmt: Codeunit "EOS089 WMS Return Values Mgmt.";
        ScanNotValidErr: Label 'Scan %1 is not a valid Sales Order', Comment = '%1: Scan Document No.';
    begin
        // Manage search and creation of a custom activity line to use as a source for incoming scan
        EOS089WMSActScanDetail.TestField("Scan Document No.");
        ...
        SalesHeader.SetRange("No.", EOS089WMSActScanDetail."Scan Document No.");
        if SalesHeader.IsEmpty() then begin
            EOS089WMSReturnValuesMgmt.PrepareReturnValues();
            EOS089WMSReturnValuesMgmt.SetRecordSearchCustomError(StrSubStNo(ScanNotValidErr, EOS089WMSActScanDetail."Scan Document No."));
            ReturnValues := EOS089WMSReturnValuesMgmt.GetReturnValues();
            exit(false);
        end;

        EOS089WMSCustomActLine.CopyFilters(EOS089WMSCustomActLine_Internal);
        if EOS089WMSActScanDetail."Source Line No." <> 0 then
            EOS089WMSCustomActLine.SetRange("Line No.", EOS089WMSActScanDetail."Source Line No.");
        EOS089WMSCustomActLine.SetRange("Scan Document No.", EOS089WMSActScanDetail."Scan Document No.");
        if EOS089WMSCustomActLine.IsEmpty() then begin
            ...
            EOS089WMSCustomActLine."Scan Document No." := EOS089WMSActScanDetail."Scan Document No.";
            EOS089WMSCustomActLine.Modify();
        end else begin
            EOS089WMSReturnValuesMgmt.PrepareReturnValues();
            EOS089WMSReturnValuesMgmt.SetRecordSearchCustomError('Document already scanned');
            ReturnValues := EOS089WMSReturnValuesMgmt.GetReturnValues();
            exit(false);
        end;
    ...
    begin
        ...
        EOS089WMSScannerSetup.Description := 'Document No. Example';
        ...
        EOS089WMSScannerDetail."Barcode Part" := Enum::"EOS089 WMS Barcode Part"::DocumentNo;
