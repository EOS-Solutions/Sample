...
/// Use OnBeforeGenerateHookList or OnAfterGenerateHookList to add your own custom formats code using AddNewEntry function of HookListTMP
/// </summary>
codeunit 50000 "Outb. EDoc. Cust. Format"
{
                ...
                'OTTIMAX':
                    begin
                        if not NotDirectShip then begin
                            ...
                            orderNo := line.Field(BufferTMP.FieldNo("EOS Doc No.")).Value();
    ...
    begin
        ...
        case OutbElectrDocSetupGroup."EOS Hook Group Code" of
            ...
            'LEROY':
                if headerLine.Number() in [DATABASE::"Sales Invoice Line", DATABASE::"Sales Cr.Memo Line"] then begin
                    Clear(DocumentTMP);
            ...
            'ESSELUNGA':
                if headerLine.Number() in [DATABASE::"Sales Invoice Line", DATABASE::"Sales Cr.Memo Line"] then begin
                    ...
                    if headerLine.Number() = DATABASE::"Sales Invoice Line" then begin
                        DataTypeManagement.FindFieldByName(headerLine, FldRef, SalesInvoiceLine.FieldName("Shipment No."));
                        ...
                        if headerLine.Number() = DATABASE::"Sales Cr.Memo Line" then begin
                            DataTypeManagement.FindFieldByName(headerLine, FldRef, SalesCrMemoLine.FieldName("Return Receipt No."));
                            DocumentTMP."Order No." := FldRef.Value();
            ...
            'CAMST':
                if headerLine.Number() in [DATABASE::"Sales Invoice Line", DATABASE::"Sales Cr.Memo Line"] then begin
                    Clear(DocumentTMP);
                    if DataTypeManagement.FindFieldByName(headerLine, FldRef, DocumentLineTMP.FieldName("Dimension Set ID")) then
    ...
    local procedure OnCheckDatiGenerali(header: RecordRef; OrderNoBuffer: Record "EOS RifLineNo Buffer" temporary)
    var
        DocumentTMP: Record "Sales Invoice Header" temporary;
        CustNo: code[20];
    begin
    ...
    local procedure AMAZON_OnCheckDatiGenerali(header: RecordRef; OrderNoBuffer: Record "EOS RifLineNo Buffer" temporary)
    var
    ...
    begin
        ...
        IF DocumentLineTMP.Type = DocumentLineTMP.Type::Item THEN begin
            ...
            ItemCrossReference.SETRANGE("Item No.", DocumentLineTMP."No.");
            ItemCrossReference.SETRANGE("Variant Code", DocumentLineTMP."Variant Code");
            ItemCrossReference.SETRANGE("Cross-Reference Type", ItemCrossReference."Cross-Reference Type"::"Bar Code");
            ItemCrossReference.SETRANGE("Unit of Measure", DocumentLineTMP."Unit of Measure Code");
            ItemCrossReference.SETRANGE("Cross-Reference Type No.", OutbElectrDocSetupGroup."EOS Hook Group Code");
