report 18123355 "EOS Customer Statement"
{
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            begin
                if ParametersBuffer."Primary Key" <> '' then
                    if ParametersBuffer."Customer Vendor Table Filter 1" <> '' then begin
                        Customer.Reset();
                        Customer.SetView(ParametersBuffer."Customer Vendor Table Filter 1");
                    end;
        ...
        dataitem(SalespersonFilters; "Salesperson/Purchaser")
        {
            ...
            begin
                if ParametersBuffer."Primary Key" <> '' then
                    if ParametersBuffer."SalesPerson Table Filter 1" <> '' then begin
                        SalespersonFilters.Reset();
                        SalespersonFilters.SetView(ParametersBuffer."SalesPerson Table Filter 1");
                    end;
                CurrReport.Break();
            end;
        }
        dataitem(DataProcessing; Integer)
        {
            ...
            begin
                ...
                if ShowPreviousBalancePrmtr = "EOS008 Show Previous Balance"::"Defined by setup" then
                    ShowPreviousBalancePrmtr := AdvCustVendStatSetup."Show Previous Balance";
                ...
                AssetsEngine.BuildMultiSourceTreeView("EOS008 Source Type"::Customer, Customer.GetView(false), "EOS008 Date Filter Type"::"Posting Date", StartingPostingDate, EndingPostingDate,
                                                         StartingDueDate, EndingDueDate, OnlyOpenPrmtr, false, '', TempReportingBuffer[1]);

                OnAfterBuildMultiSourceTreeView(Customer.GetView(false), "EOS008 Date Filter Type"::"Posting Date", StartingPostingDate, EndingPostingDate,
                                                StartingDueDate, EndingDueDate, OnlyOpenPrmtr, false, '', TempReportingBuffer[1]);
                ...
                if not UseSalespersonFromCustomerPrmtr then
                    if SalespersonFilters.GetFilters() <> '' then begin
                        SalespersonFilter := GetSelectionFilterForSalesperson(SalespersonFilters);
                        AssetsEngine.ApplySalespersonFilters(TempReportingBuffer[1], SalespersonFilter);
                    end;

                TempReportingBuffer[2].SetCurrentKey("EOS Level No.");
                TempReportingBuffer[2].SetRange("EOS Level No.", 2);
                if TempReportingBuffer[2].FindSet(true) then
                    repeat
                        Clear(TempReportingBuffer[4]);
                        TempReportingBuffer[4].SetCurrentKey("EOS Node Linked To");
                        ...
                        if ShowLinkedEntriesPrmtr then begin
                            ...
                            TempReportingBuffer[3].SetCurrentKey("EOS Node Linked To");
                ...
                Clear(TempReportingBuffer);
                TempReportingBuffer[1].SetCurrentKey("EOS Level No.");
                ...
                if TempReportingBuffer[2].FindSet() then
                    repeat
                        Clear(TempReportingBuffer[3]);
                        TempReportingBuffer[3].SetCurrentKey("EOS Node Linked To", "EOS Document Type");
                        TempReportingBuffer[3].SetRange("EOS Node Linked To", TempReportingBuffer[2]."EOS Entry No.");
                ...
                OnAfterBuildReportingDataset(Customer.GetView(false), "EOS008 Date Filter Type"::"Posting Date", StartingPostingDate, EndingPostingDate,
    ...
    requestpage
    {
        ...
        layout
        {
            area(Content)
            {
                ...
                field(ShowPreviousPeriod; ShowPreviousBalancePrmtr)
                {
                    Caption = 'Show Previous Balance';
                    ApplicationArea = All;
                    ToolTip = 'Specifies whether to show the previous period balance in the statement when a date filter in the format "date.." is specified.';
                }
        ...
        trigger OnOpenPage()
        begin
            SubscriptionMgt.GetSubscriptionIsActive(); // this will raise a notification if subscription is not active
            SetReportParameters();
        end;
    ...
    protected var
        ...
        ShowPreviousBalancePrmtr: Enum "EOS008 Show Previous Balance";
...
#pragma warning restore AL0432
    ...
    begin
        ...
        AdvCustVendStatRoutines.GetPaymentTermsDescription(CustomerPrint."Payment Terms Code", CustomerPrint."Language Code");
    ...
    begin
        if AdvCustVendStatSharedMem.GetReportParameter(Parameters) then begin
            ...
            ShowPreviousBalancePrmtr := Parameters."Show Previous Balance";
            ...
            ParametersBuffer."Primary Key" := 'SAVED';
        end;
    end;
